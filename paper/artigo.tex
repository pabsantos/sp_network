...
\title{Topological characteristics of urban road networks - A case study in São Paulo, Brazil}
...
%Urban road networks are critical infrastructure systems susceptible to performance degradation caused by natural disasters, particularly floods. This work investigates the vulnerability of the urban road network in the Tamanduate\'i river basin, S\~ao Paulo, Brazil, to flooding events. The road network was modeled as a graph with 21,045 nodes and 31,615 undirected edges, extracted from 92 origin/destination zones. Average degree, clustering coefficient, and average shortest path length were computed and compared with an equivalent random network. Edge betweenness centrality (EBC) was compared with flood occurrence data (2022--2025) and population distribution from the 2022 Brazilian Census. The real network exhibited a clustering coefficient approximately 420 times higher and an average shortest path length 7.4 times longer than the random equivalent, and edges with EBC values in the ranges 0.06--0.07 and 0.12--0.13 concentrated the highest normalized flood counts. Population distribution showed little variation across most EBC ranges. These results indicate that specific road segments are considerably more vulnerable to flooding, while topological vulnerability and population exposure are not strongly correlated across the network.

\noindent
%{\bf Keywords}. complex networks, road network vulnerability, floods, edge betweenness centrality, S\~ao Paulo
\end{abstract}

\section{Introduction}

Road networks are vulnerable to disasters, such as floods, which can cause damage to infrastructure and adversely affect travel and/or local populations on degraded networks. Studying and analyzing the topological characteristics of road networks helps prioritize planning, budgeting, and maintenance, as well as prepare effective emergency response plans \cite{balijepalliMeasuringVulnerabilityRoad2014}. Each city has its own characteristics of road design, intersection density, mobility patterns, and population distribution, which can directly influence results.
...
% Mais um parágrafo aqui sobre networks
...
%The loaded road network graph has 21,045 nodes and 48,769 edges (31,615 undirected). Table \ref{tab:network-comparison} compares the network parameters with an equivalent random network $G(N, p)$. The average clustering coefficient $\langle c \rangle$ of the real network is approximately 420 times higher than the random equivalent, indicating a significantly clustered structure. The average shortest path length $\langle l \rangle$ of the real network is approximately 7.4 times higher than the random network.

% \begin{table}[H]
% \centering
% \caption{Network parameters: real network vs. equivalent random network}
% \label{tab:network-comparison}
% \begin{tabular}{lrr}
% \hline
% Parameter & Real & Random \\
% \hline
% $\langle k \rangle$  & 3.0045  & 3.0045 \\
% $\langle c \rangle$  & 0.0600  & 0.0001 \\
% $\langle l \rangle$  & 67.0786 & 9.0485 \\
% \hline
% \end{tabular}
% \end{table}

% Figure \ref{fig:power} shows the probability of node degree per degree value. There is a high probability of degree values between 2 and 6. The values vary between 1 and 10. The network does not strictly follow a power-law behavior.

% \begin{figure}[H]
% \centering
% \includegraphics[width=.7\textwidth]{../plot/degree_distribution.png}
% \caption{Node degree distribution. Source: The authors.}
% \label{fig:power}
% \end{figure}
...
% \caption{Network EBC results. Source: The authors.}
% \label{fig:ebcmap}
% \end{figure}
...
% \caption{EBC and normalized flood per edge. Source: The authors.}
...
% \caption{EBC and normalized population per edge. Source: The authors.}
